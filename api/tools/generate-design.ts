// Vercel Serverless Function: POST /api/tools/generate-design
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!initResp.ok) {
      ...
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: 'MCP initialize failed', status: initResp.status, details: text }));
      return;
    }
    ...
    if (!callResp.ok) {
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: 'MCP tools/call failed', status: callResp.status, details: callJson }));
      return;
    }
    ...
    if (typeof text !== 'string') {
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: 'Unexpected MCP response shape', details: callJson }));
      return;
    }
    ...
    try {
      ...
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: 'Failed to parse MCP tool text as JSON', rawText: text }));
      return;
    }
    ...
    if (!imageUrl) {
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: 'No image URL found in MCP output', details: { parsed, body } }));
      return;
    }
